...
type dbFile struct {
	Users map[string]*UserSeries `json:"users"`
}

// Store 持久化用户流量（保留约 1 年 hourly + 近 7 日 5min）
type Store struct {
	path string
	mu   sync.Mutex
	data dbFile
...
func DefaultStore() *Store {
	if defaultStore == nil {
		defaultStore = NewStore(filepath.Join("/var/lib/qosnat2", "ocserv-user-traffic.json"))
	}
	return defaultStore
}
...
func NewStore(path string) *Store {
	s := &Store{path: path, data: dbFile{Users: map[string]*UserSeries{}}}
	_ = s.load()
	return s
}
...
func (s *Store) user(name string) *UserSeries {
	if s.data.Users == nil {
		s.data.Users = map[string]*UserSeries{}
	}
...
// RecordCounters 根据 occtl 当前 RX/TX 计数累加增量
func (s *Store) RecordCounters(username string, rx, tx uint64, now time.Time) error {
