...
# 发布: 创建 tag ocserv-<官方版本> 并上传该 tar.gz 为 release 资产。
#
# 环境变量:
#   OCSERV_UPSTREAM_TAG  ocserv 官方源码 tag（GitLab，默认 1.4.2）；同时作为发布包版本号
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT_DIR="${ROOT}/dist"
VERSION="${OCSERV_UPSTREAM_TAG:-1.4.2}"
VERSION="${VERSION#v}"
...
log "编译安装 ocserv 官方版本 ${VERSION} 到临时前缀 ${STAGE}/prefix ..."
OCSERV_TAG="${VERSION}" \
