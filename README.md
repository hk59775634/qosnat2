...
| **VPN** | WireGuard；**ocserv**（OpenConnect，[`scripts/install-ocserv.sh`](scripts/install-ocserv.sh) 官方 1.5.0 + Route B SPEC-01 + DST SPEC-02） |
...
>
> **qosnat2 版本号**：`YYYYMMDD` + 每日 2 位序号（如 `2026052801`），见 [`releases/qosnat2-versions.json`](releases/qosnat2-versions.json)；Web 可从 GitHub 拉取清单并切换。**ocserv** 无版本切换，仅支持源码编译安装（官方 tag **1.5.0** + SPEC-01 + DST）。
