...
const isDirector = computed(() => (cfg.value.role || 'director') === 'director')
const isRS = computed(() => cfg.value.role === 'rs')
...
function ensureRsEntries() {
  if (!cfg.value.rs) cfg.value.rs = { entries: [] }
  if (!cfg.value.rs.entries) cfg.value.rs.entries = []
}
...
async function applyAll() {
  ...
  err.value = ''
  try {
    if (cfg.value.role === 'rs') cfg.value.mode = 'dr'
...
async function addRSBinding() {
  err.value = ''
  ok.value = ''
  ensureRsEntries()
  const entry = { ...rsBindingForm.value }
  if (!entry.vip?.trim()) {
    err.value = t('nat.lvs.needRealServers')
    return
  }
  cfg.value.rs.entries = [...cfg.value.rs.entries, entry]
  cfg.value.enabled = true
  try {
    await api.lvs.put(cfg.value)
    await api.lvs.apply()
    ok.value = t('nat.lvs.rsBindingAdded')
    rsBindingForm.value = { vip: '', port: rsBindingForm.value.port, protocol: 'tcp_udp', comment: '' }
    await load()
  } catch (e) {
    cfg.value.rs.entries = cfg.value.rs.entries.slice(0, -1)
    err.value = e.message
  }
}

async function removeRSBinding(id) {
  if (!confirm(t('nat.lvs.confirmDeleteRsBinding'))) return
  err.value = ''
  ensureRsEntries()
  const kept = cfg.value.rs.entries.filter((e) => e.id !== id)
  const prev = cfg.value.rs.entries
  cfg.value.rs.entries = kept
  if (kept.length === 0) cfg.value.enabled = false
  try {
    await api.lvs.put(cfg.value)
    await api.lvs.apply()
    await load()
  } catch (e) {
    cfg.value.rs.entries = prev
    err.value = e.message
  }
}
...
function rsProtocolLabel(e) {
  if (e.protocol === 'tcp_udp') return 'TCP + UDP'
  return e.protocol
}
...
<template>
  <div class="page-stack">
    ...
    <div v-for="(w, i) in warnings" :key="i" class="card card-body text-xs text-amber-800 bg-amber-50">
      <template v-if="isDirector">{{ t('nat.lvs.localOcservWarn') }} {{ w }}</template>
      <template v-else>{{ t('nat.lvs.rsOcservHint') }} {{ w }}</template>
    </div>
    ...
    <div class="card card-body space-y-3 text-sm">
      <div class="flex flex-wrap items-center gap-2 text-xs">
        ...
        <template v-else>
          <span class="text-xs text-slate-500">{{ t('nat.lvs.rsStatus') }}:</span>
          <span
            class="inline-flex px-2 py-0.5 rounded"
            :class="rsStatus.active ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'"
          >
            {{ rsStatus.active ? t('nat.lvs.rsActive') : t('nat.lvs.rsInactive') }}
          </span>
          <span v-if="rsStatus.summary" class="text-slate-500">{{ rsStatus.summary }}</span>
          <span v-if="devLan" class="text-slate-400">LAN: {{ devLan }}</span>
        </template>
      </div>
      <p class="text-xs text-slate-600">{{ t('nat.lvs.modeHint') }}</p>
      <div v-if="isDirector && !status.installed" class="flex gap-2">
      ...
      <div class="flex flex-wrap gap-4 items-center">
        <fieldset class="flex flex-wrap gap-3 items-center border-0 p-0">
          <legend class="sr-only">{{ t('nat.lvs.role') }}</legend>
          <span class="text-xs text-slate-500">{{ t('nat.lvs.role') }}</span>
          <label class="flex items-center gap-1.5 text-xs">
            <input v-model="cfg.role" type="radio" value="director" />
            {{ t('nat.lvs.roleDirector') }}
          </label>
          <label class="flex items-center gap-1.5 text-xs">
            <input v-model="cfg.role" type="radio" value="rs" />
            {{ t('nat.lvs.roleRS') }}
          </label>
        </fieldset>
        ...
        <label v-if="isDirector" class="flex items-center gap-2">
        ...
        <span v-else class="text-xs text-slate-500">DR</span>
    ...
    <template v-if="isRS">
      <div class="card card-body space-y-3 text-sm">
        <h3 class="font-medium">{{ t('nat.lvs.rsBindings') }}</h3>
        <p class="text-xs text-slate-600">{{ t('nat.lvs.roleRSHint') }}</p>
        <p class="text-xs text-slate-500">{{ t('nat.lvs.rsBindingsHint') }}</p>
        <form class="grid md:grid-cols-2 lg:grid-cols-4 gap-3 items-end" @submit.prevent="addRSBinding">
          <div>
            <label class="text-xs text-slate-500">VIP</label>
            <input v-model="rsBindingForm.vip" class="input-field font-mono mt-0.5" placeholder="203.0.113.10" />
          </div>
          <div>
            <label class="text-xs text-slate-500">{{ t('nat.lvs.port') }}</label>
            <input v-model.number="rsBindingForm.port" type="number" min="1" max="65535" class="input-field mt-0.5" />
          </div>
          <div>
            <label class="text-xs text-slate-500">{{ t('nat.forwards.proto') }}</label>
            <select v-model="rsBindingForm.protocol" class="input-field mt-0.5">
          ...
          <div>
            <button type="submit" class="btn-secondary">{{ t('nat.lvs.addRsBinding') }}</button>
          </div>
        </form>
      </div>
      ...
      <div class="card table-wrap card-body !p-2">
        <table class="data w-full text-sm">
          ...
          <tbody>
            <tr v-for="e in cfg.rs?.entries || []" :key="e.id">
              <td class="font-mono">{{ e.vip }}</td>
              <td>{{ e.port }}</td>
              <td>{{ rsProtocolLabel(e) }}</td>
              <td class="text-right">
                <button type="button" class="btn-danger" @click="removeRSBinding(e.id)">{{ t('common.delete') }}</button>
              </td>
            </tr>
            <tr v-if="!(cfg.rs?.entries || []).length">
              <td colspan="4" class="text-center text-slate-400 py-6">{{ t('nat.lvs.noRsBindings') }}</td>
            </tr>
          </tbody>
        </table>
      </div>
